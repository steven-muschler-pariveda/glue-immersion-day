...
## @return: DataSource0
## @inputs: []
Updates = glueContext.create_dynamic_frame.from_options(format_options = {"jsonPath":"","multiline":False}, connection_type = "s3", format = "json", connection_options = {"paths": ["s3://INSERT_BUCKET_NAME/dms_updates/"], "recurse":True}, transformation_ctx = "Updates")

Records = spark.read.load("s3a://INSERT_BUCKET_NAME/dms_parquet/")
#= glueContext.create_dynamic_frame.from_options(format_options = {"jsonPath":"","multiline":False}, connection_type = "s3", format = "parquet", connection_options = {"paths": ["s3://INSERT_BUCKET_NAME/dms_parquet/"], "recurse":True}, transformation_ctx = "Records").toDF()
## @type: ApplyMapping
...
MergedDataFrame.write.option("compression","gzip").partitionBy("year_month").parquet("s3://INSERT_BUCKET_NAME/dms_temp/customer/",mode="overwrite")

Output = spark.read.load("s3a://INSERT_BUCKET_NAME/dms_temp/")

Output.write.option("compression","gzip").partitionBy("year_month").parquet("s3://INSERT_BUCKET_NAME/dms_parquet/customer/",mode="overwrite")
